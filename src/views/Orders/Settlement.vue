    ...
    <div class="settlement">
        ...
        <div class="view-body" v-if="orderInfo">
            ...
            >{{userInfo.address}}{{userInfo.bottom}}</span>
                        <span v-else>
              <span v-if="hasAddress" @click="$router.push('/myAddress')">选择收货地址</span>
              <span v-else @click="addAddress">新增收货地址</span>
                ...
                <section class="checkout-section">
                    <CartItem :subHead="remarkInfo.tableware || '未选择'" @click="showTableWare=true" title="餐具份数"/>
                    <CartItem @click="$router.push('/remark')" title="订单备注" :subHead="remarkInfo.remark||'口味 偏好'"/>
                    <CartItem title="发票信息" subHead="不需要开发票"/>
                </section>

                <!--                餐具分数-->
                <Tableware @close="showTableWare=false" :isShow="showTableWare"/>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                hasAddress: false, //新增收货地址开关
                showTableWare: false//餐具开关
            };
        },
        components: {
            Header,
