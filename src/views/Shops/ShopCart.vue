...
<script>
    ...
    export default {
        ...
        data() {
            return {
                totalCount: 0,//点餐食物个数。这里并不是使用监听来实现的数量变化，而是依靠的计算属性isEmpty在判断是否为空时顺便计算出来
                totalPrice: 0,//点餐总价。这里并不是使用监听来实现的数量变化，而是依靠的计算属性isEmpty在判断是否为空时顺便计算出来
                selectFoods: [],//已选商品
                showCartView: false //是否显示购物车详情（弹出框）
            }
        },
        props: {
            shopInfo: Object//所进入的商店
        },
        computed: {
            isEmpty() { //购物车是否为空
                let empty = true;
                ...
                this.shopInfo.recommend.forEach(recommend => {
                    recommend.items.forEach(item => {
                        if (item.count) {
                            empty = false; //如果父组件中已经有了count值说明点了食品，则不再为空
                            this.totalCount += item.count;//开始计算每个count值并累加
                ...
                this.shopInfo.menu.forEach(menu => {
                    menu.foods.forEach(food => {
                        if (food.count) {
                            empty = false; //如果父组件中已经有了count值说明点了食品，则不再为空
                            this.totalCount += food.count;//开始计算每个food值并累加
