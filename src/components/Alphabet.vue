    ...
    <div class="area" v-if="cityInfo" ref="area_scroll">
        <div class="scroll_wrap">
            ...
            <div class="hot_wrap citylist">
                ...
                <ul class="hot_city">
                    <li @click="$emit('selectCity',item)" v-for="(item,index) of cityInfo.hotCities" :key="index">{{item.name}}</li>
                </ul>
            </div>
            <!--            所有城市-->
            <div class="city_wrap">
                ...
                <div class="city_content citylist" v-for="(item,index) of keys" :key="index">
                    ...
                    <!--                    根据字母展示城市名-->
                    <ul>
                        <li @click="$emit('selectCity',city)" v-for="(city,index) of cityInfo[item]" :key="index">
                            {{city.name}}
                        </li>
                    </ul>
                </div>
        ...
        <div class="area_keys">
            <ul>
                ...
                <li @click="selectKey(index+1)" v-for="(item,index) of keys" :key="index">{{item}}</li>
            </ul>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            selectKey(index) {
                // 我为热门节点div和所有城市节点添加了一个class为citylist的标识，这个列表获取到之后就可以根据下标来跳转到指定元素了。
